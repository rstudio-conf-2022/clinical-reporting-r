...
# are +0 rounding and the quantiles are +1 rounding
new_param_set <- param_set("{min}, {max}" = c(0, 0), 
                           "{q1}, {q3}" = c(1, 1), 
                           "missing" = NA)
...
pft_tfrmt <- tfrmt_sigdig(sigdig_df,
             group = vars(pfts, visit), 
             label = label,
             param_defaults = new_param_set) 
